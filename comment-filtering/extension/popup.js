document.addEventListener("DOMContentLoaded", function () {
    ...
    chrome.identity.getAuthToken({ interactive: true }, function (token) {
        ...
        .then(res => {
            if (!res.ok) {
                return res.text().then(text => { throw new Error(`Google API ${res.status}: ${text}`); });
            }
            return res.json();
        })
        .then(data => {
    ...
    addWordButton.addEventListener("click", function () {
        ...
        .then(res => {
            if (!res.ok) {
                return res.text().then(text => { throw new Error(`API ${res.status}: ${text}`); });
            }
            return res.json();
        })
        .then(data => {
    ...
    function loadUserBadWords(userId) {
        fetch(`http://127.0.0.1:8000/user_badwords/${userId}`)
            ...
            .then(data => {
                wordList.innerHTML = "";
                if (Array.isArray(data.bad_words) && data.bad_words.length > 0) {
                    ...
                    wordList.innerHTML = '<p>등록된 금칙어가 없습니다.</p>';
                }
            })
            .catch(err => {
                console.error("금칙어 목록 불러오기 오류:", err);
                wordList.innerHTML = `<p>금칙어 목록 로딩 중 오류가 발생했습니다.<br>${err.message}</p>`;
            });
    }
