...
# 금칙어 추가
@app.post("/badwords")
def add_bad_word(request: BadWordRequest):
    with engine.connect() as conn:
        conn.execute(text("INSERT INTO bad_words (word, category) VALUES (:word, :category)"),
                     {"word": request.word, "category": request.category})
        conn.commit()
    return {"message": f"'{request.word}' 금칙어 추가 완료!"}

# 금칙어 삭제
@app.delete("/badwords/{word}")
def delete_bad_word(word: str):
    with engine.connect() as conn:
        conn.execute(text("DELETE FROM bad_words WHERE word = :word"), {"word": word})
        conn.commit()
